...
export class ReignCompanySheet extends ActorSheet {
  /** @override */
  static get defaultOptions() {
    const options = super.defaultOptions;
    options.classes = ["reign", "sheet", "actor"];
    options.template = "systems/reign-vtt/templates/company-sheet.html";
    options.width = 800;
    options.height = 700;
  ...
  getData() {
    ...
    console.log("ReignCompanySheet: getData called", context);
    return context;
  }
  ...
  activateListeners(html) {
    ...
    console.log("ReignCompanySheet: activateListeners called", html);
    ...
    element.find('.number-attribute').change(ev => {
      ...
      this.actor.update({ [target.name]: value }).then(() => {
        console.log("Company Actor updated:", this.actor);
        console.log("Updated attribute value:", this.actor.system.attributes[target.name.split('.')[2]].value);
      }).catch(err => {
        console.error("Error updating company actor:", err);
      });
    });
    ...
    element.find('.skill-row-template input[type="text"]').change(ev => {
      ...
      if (skillName) {
        ...
        this.actor.update(updateData).then(() => {
          console.log("Company Actor updated with new skill:", this.actor);
        }).catch(err => {
          console.error("Error updating company actor with new skill:", err);
        });
...
Hooks.once('init', async function () {
  try {
    ...
    Actors.registerSheet("reign", ReignCompanySheet, { makeDefault: true });

    registerHelpers();

    console.log("ReignCompanySheet: Templates loaded and sheet registered");
  } catch (err) {
    console.error("Error initializing ReignCompanySheet:", err);
  }
});
